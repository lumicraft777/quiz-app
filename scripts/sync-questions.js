...
async function main() {
  ...
  // 太陽光発電の基礎知識（製品データに依存しない静的問題。初級〜上級混在）
  const solarBasicsQuestions = generateSolarBasicsQuestions();
  console.log(`  [太陽光発電] ${solarBasicsQuestions.length}問を生成`);
  knowledgeQuestions = knowledgeQuestions.concat(solarBasicsQuestions);

  // 第2章 システム設計・発電量計算（製品データに依存しない静的問題）
  const systemDesignQuestions = generateSystemDesignQuestions();
  console.log(`  [システム設計・発電量計算] ${systemDesignQuestions.length}問を生成`);
  knowledgeQuestions = knowledgeQuestions.concat(systemDesignQuestions);

  // 第3章 機器・部品の知識（製品データに依存しない静的問題）
  const equipmentQuestions = generateEquipmentQuestions();
  console.log(`  [機器・部品の知識] ${equipmentQuestions.length}問を生成`);
  knowledgeQuestions = knowledgeQuestions.concat(equipmentQuestions);

  // 第4章 施工・設置の知識（製品データに依存しない静的問題）
  const constructionQuestions = generateConstructionQuestions();
  console.log(`  [施工・設置の知識] ${constructionQuestions.length}問を生成`);
  knowledgeQuestions = knowledgeQuestions.concat(constructionQuestions);
  ...
  // 製品詳細パネル用の products テーブルも同時に更新する
  // （同じメーカー×シリーズが複数シートに出てくる場合は後勝ちで統合）
  const productMap = new Map();
  allProducts.forEach((p) => {
    if (!p.maker || !p.series) return;
    productMap.set(`${p.maker} ${p.series}`, {
      maker: p.maker,
      series: p.series,
      feature: isUnknownValue(p.feature) ? null : p.feature,
      demerit: isUnknownValue(p.demerit) ? null : p.demerit
    });
  });
  const productRows = Array.from(productMap.values());
  console.log(`productsテーブルを全件削除中...（${productRows.length}製品を書き込み予定）`);
  await supabaseRequest("products?maker=not.is.null", { method: "DELETE" });

  console.log("productsテーブルへ書き込み中...");
  const productBatches = chunk(productRows, 200);
  for (let i = 0; i < productBatches.length; i++) {
    await supabaseRequest("products", {
      method: "POST",
      headers: { Prefer: "return=minimal" },
      body: JSON.stringify(productBatches[i])
    });
    console.log(`  ${Math.min((i + 1) * 200, productRows.length)} / ${productRows.length} 件 書き込み完了`);
  }
